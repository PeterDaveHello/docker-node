...
- Regenerate the official manifest with `node stackbrew.js > ../official-images/library/node` before proposing downstream updates; direct the output to a sibling clone of `docker-library/official-images` (for example, `git clone https://github.com/docker-library/official-images ../official-images`). The script consumes `versions.json` and the tracked Dockerfiles.
- Documentation checks mirror CI: run `doctoc --title='## Table of Contents' --github README.md && doctoc --title='## Table of Contents' --github docs/BestPractices.md` (see `.github/workflows/doctoc.yml`) and `find . -name "*.md" | xargs -n 1 markdown-link-check -c markdown_link_check_config.json -q` (see `.github/workflows/markdown-link-check.yml`).
